...
export function localDateTimeToUtc(dateYYYYMMDD: string, timeHHmm: string, tz: string): number {
  // Validate inputs
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateYYYYMMDD)) throw new Error("Invalid date format, expected YYYY-MM-DD");
  if (!/^\d{2}:\d{2}$/.test(timeHHmm)) throw new Error("Invalid time format, expected HH:mm");
  ...
  const get = (type: string) => parts.find(p => p.type === type)?.value as string | undefined;
...
export function gymDayBoundsUtc(dateYYYYMMDD: string, tz: string): { from: number; to: number } {
